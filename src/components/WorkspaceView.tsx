...
import { saveFile, getFile, deleteFile, updateFileExpiresAt } from "../utils/db";
import {
  packageFolderToZip,
...
export default function WorkspaceView({
  ...
  const handleOpenTransfer = onOpenTransferTab || onNavigateToTransfer || (() => {});
  ...
  const [sharingFile, setSharingFile] = useState<ShareableFileData | null>(null);
  ...
  const showToast = (text: string, type: "error" | "info" | "success" = "error") => {
    setToastMessage({ text, type });
    setTimeout(() => setToastMessage(null), 4000);
  };

  // In-app delete confirmation modal states (100% reliable in sandboxed iframe)
  const [fileToDelete, setFileToDelete] = useState<WorkspaceFileMetadata | null>(null);
  const [isDeletingFile, setIsDeletingFile] = useState(false);
  const [folderToDelete, setFolderToDelete] = useState<WorkspaceFolder | null>(null);
  ...
  const refreshWorkspace = async () => {
    ...
    try {
      const remoteWs = await fetchWorkspaceFromSupabase(session.workspaceId, session.email || "");
      if (remoteWs) {
  ...
  const usage = session?.workspaceId
    ...
    : { usedBytes: 0, limitBytes: 10 * 1024 * 1024 * 1024, percentage: 0, fileCount: 0 };

  const handleSaveName = async () => {
    if (editNameInput.trim() && session.workspaceId) {
      const newName = editNameInput.trim();
      updateWorkspaceName(session.workspaceId, newName);
  ...
  const handleCreateFolder = async (e: React.FormEvent) => {
    ...
    await addFolderToWorkspaceInSupabase(session.workspaceId, name);
    refreshWorkspace();
  };
  ...
  const handleDrop = async (e: React.DragEvent) => {
    ...
    if (e.dataTransfer.items && e.dataTransfer.items.length > 0) {
      try {
        const folderResult = await getFilesFromDataTransfer(e.dataTransfer.items);
        if (folderResult.isFolder && folderResult.files.length > 0) {
          await processAndUploadFolder(folderResult.files, folderResult.folderName);
          return;
  ...
  const uploadSingleFile = async (file: File) => {
    ...
    try {
      ...
      setUploadProgress((prev) => ({ ...prev, percent: 100, statusMessage: "Saving to workspace..." }));
      ...
      await saveWorkspaceFileToSupabase(session.workspaceId, session.email || "", fileMeta);
      refreshWorkspace();

      setTimeout(() => {
        setUploadProgress({ active: false, fileName: "", percent: 0, speed: "", loaded: "" });
      }, 700);
    } catch (err: any) {
      clearInterval(interval);
  ...
  const processAndUploadFolder = async (fileItems: FileWithPath[], folderName: string) => {
    ...
    try {
      const zipFile = await packageFolderToZip(fileItems, folderName, (zipPercent) => {
        setUploadProgress((prev) => ({
          ...prev,
      ...
      setUploadProgress((prev) => ({ ...prev, percent: 100, statusMessage: "Saving folder to workspace..." }));
      ...
      showToast(`Folder packaging failed: ${err?.message || "Please check files"}`);
      setUploadProgress({ active: false, fileName: "", percent: 0, speed: "", loaded: "" });
    }
  };
  ...
  const confirmDeleteFile = async () => {
    ...
    try {
      ...
      showToast(`"${targetName}" permanently deleted from your workspace.`, "success");
    } catch (err) {
  ...
  const confirmDeleteFolder = async () => {
    ...
    try {
      ...
      showToast(`Folder "${targetName}" deleted. Files moved to All Files.`, "success");
    } catch (err) {
  ...
  const getRemainingTimeStr = (
    ...
    if (isPaused) {
      if (pausedRemainingMs && pausedRemainingMs > 0) {
        ...
        const mins = Math.floor((pausedRemainingMs % (1000 * 60 * 60)) / (1000 * 60));
      ...
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        <div className="p-8 sm:p-12 rounded-3xl border shadow-2xl bg-[#18181c] border-gray-800 text-white">
          ...
          <p className="text-sm text-gray-400 mb-8 max-w-md mx-auto leading-relaxed">
            All member workspace files and folders are securely stored in the database. When you want to use your workspace, sign in with your member email and password.
          </p>
  ...
  return (
    ...
    >
      ...
      <input
        ...
        type="file"
        onChange={(e) => {
          if (e.target.files && e.target.files[0]) uploadSingleFile(e.target.files[0]);
          e.target.value = "";
        }}
        className="hidden"
      />
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            ...
            <div>
              <div className="flex items-center space-x-2">
                {isEditingName ? (
                  ...
                  <div className="flex items-center space-x-2">
                    <h2 className="text-xl sm:text-2xl font-black tracking-tight">{workspaceName}</h2>
                    <button
                      onClick={() => {
              ...
              <p className="text-xs text-gray-400 mt-1 flex items-center space-x-2">
                <span className="truncate max-w-[200px] sm:max-w-none">Account: {session?.email || "Registered User"}</span>
          ...
          <div className="w-full lg:w-72 bg-gray-500/5 p-4 rounded-2xl border border-gray-500/10">
            <div className="flex items-center justify-between text-xs font-semibold mb-1.5">
              ...
              <span className="font-mono text-cyan-400 font-bold">{usage.percentage}% used</span>
            </div>
        ...
        <div className="mt-6 pt-6 border-t border-gray-500/15 flex flex-wrap items-center gap-3">
          ...
          {files.length > 0 && (
            ...
            >
              ...
              <span>
                {areAllFilesPaused ? "Resume Expiry (All Files)" : "Pause Expiry (Freeze All)"}
              </span>
            </button>
          )}
          ...
          >
            <div className="flex items-center justify-between text-xs font-bold text-white mb-2">
              <span className="flex items-center space-x-2">
                ...
                <span className="text-gray-400 font-normal">({uploadProgress.statusMessage})</span>
              </span>
              <span className="font-mono text-cyan-400">{uploadProgress.percent}%</span>
            </div>
      ...
      >
        ...
        <p className="text-xs text-gray-400 mt-1">
          Supports multi-file bundles, complete directories, videos, photos, and documents up to 10GB.
        </p>
      </div>
      ...
      {filteredFiles.length === 0 ? (
        <div className="p-12 text-center rounded-3xl border bg-[#18181c]/50 border-gray-800">
          ...
          <h4 className="text-base font-bold text-white">No files in this folder</h4>
          <p className="text-xs text-gray-400 mt-1 max-w-sm mx-auto">
            Upload single documents or entire folders to start managing your personal workspace archive.
          </p>
          <button
            onClick={() => fileInputRef.current?.click()}
            ...
            >
                  ...
                  <div className="flex items-center space-x-1.5">
                    {file.isPaused ? (
                      ...
                      <span className="inline-flex items-center space-x-1 px-2 py-0.5 rounded-md text-[10px] font-medium bg-gray-800 text-gray-300 border border-gray-700">
                        ...
                        <span>{getRemainingTimeStr(file.expiresAt, file.pausedRemainingMs, file.isPaused)}</span>
                      </span>
                    )}
                  </div>
              ...
              <div className="mt-4 pt-3 border-t border-gray-500/15 flex items-center justify-between">
                {file.isFolder ? (
                  ...
                  <span className="text-[10px] text-gray-400 uppercase font-mono">Encrypted</span>
                )}
                <div className="flex items-center space-x-2">
                  <button
                    onClick={() =>
        ...
        <div className="rounded-2xl border overflow-hidden bg-[#18181c] border-gray-800">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-gray-500/10">
              {filteredFiles.map((file) => (
                <tr key={file.code} className="hover:bg-gray-500/5 transition">
                  <td className="p-4">
                    <div className="flex items-center space-x-3">
                      {getFileIcon(file.name, file.type, file.isFolder)}
                  ...
                  <td className="p-4 text-gray-400 font-mono">{formatBytes(file.size)}</td>
                  <td className="p-4">
                    <button
                      onClick={() => handleCopyCode(file.code)}
                      ...
                      >
                        {file.isPaused ? (
                          <>
                            ...
                            <span className="hidden sm:inline">Start Expiry</span>
                          </>
                        ) : (
                          <>
                            <Pause className="h-3.5 w-3.5 text-amber-400" />
                            <span className="hidden sm:inline">Pause Expiry</span>
                          </>
                        )}
                      </button>
                    </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
            <div className="max-w-md w-full rounded-3xl p-6 border shadow-2xl bg-[#18181c] border-gray-800 text-white">
              ...
              <form onSubmit={handleCreateFolder} autoComplete="off">
                <label className="block text-xs text-gray-400 mb-1.5 font-semibold">Folder Name</label>
                <input
                  type="text"
                  required
                  autoComplete="off"
              ...
              <div className="flex items-center justify-between pb-4 border-b border-gray-500/15">
                <div className="flex items-center space-x-3 min-w-0">
                  ...
                  <div className="truncate">
                    <h3 className="text-base font-extrabold truncate">{inspectingFolder.name}</h3>
                    <p className="text-xs text-gray-400">
                      {inspectingFolder.entries.length} items preserved in archive
                    </p>
                  </div>
                </div>
                <button
                ...
                {inspectingFolder.entries.map((entry, idx) => (
                  ...
                  >
                    <div className="flex items-center space-x-2 min-w-0 truncate">
                      ...
                      <span className="truncate text-gray-300">{entry.path}</span>
                    </div>
                    {!entry.isDir && (
              ...
              <div className="flex items-start space-x-4 mb-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-1 leading-relaxed">
                    Are you sure you want to permanently remove <span className="text-gray-200 font-bold">{fileToDelete.name}</span> ({formatBytes(fileToDelete.size)})? This action removes the file permanently from the database.
                  </p>
                </div>
              </div>
            ...
            >
              <div className="flex items-start space-x-4 mb-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-1 leading-relaxed">
                    Files stored in this custom folder will not be deleted. They will automatically be moved to <span className="text-gray-200 font-bold">All Files</span>.
                  </p>
                </div>
              </div>
